...
const msg = (t, ok = true) => {
  const m = $("#formMsg");
  m.textContent = t;
  m.className = "small mt-3 " + (ok ? "text-success" : "text-danger");
};
...
async function updateIndexWithRetry(updater, tries = 3) {
  for (let i = 0; i < tries; i++) {
    const res = await fetch(`${JSON_PATH}?t=${Date.now()}`);
    const json = res.ok ? await res.json() : { items: [] };
    const items = json.items || [];

    const updatedItems = await updater(items);

    const jsonB64 = btoa(
      unescape(
        encodeURIComponent(JSON.stringify({ items: updatedItems }, null, 2))
      )
    );

    try {
      await commitFile(
        `${MD_DIR}vacantes.json`,
        jsonB64,
        `chore(jobs): update index`
      );
      return true;
    } catch (e) {
...
async function save(e) {
  e.preventDefault();
  const data = readForm();
  if (!data.titulo || !data.empresa || !data.ubicacion || !data.descripcion) {
    msg("Llena los campos obligatorios", false);
    return;
  }
  ...
  await commitFile(
    mdPath,
    mdBase64,
    editing ? `chore(jobs): update ${slug}` : `feat(jobs): add ${slug}`
  );

  // ✅ nuevo bloque con retry
  await updateIndexWithRetry((items) => {
    const meta = {
      slug,
      titulo: data.titulo,
...
async function toggleActive() {
  ...
  await updateIndexWithRetry((items) => {
    const i = items.findIndex((x) => x.slug === slug);
    if (i >= 0) items[i].activo = !items[i].activo;
    return items;
  });
...
async function removeVacante() {
  ...
  await updateIndexWithRetry((items) => items.filter((x) => x.slug !== slug));
...
$("#search").addEventListener("input", async (e) => {
  ...
  const f = (items || []).filter((v) =>
    [v.titulo, v.empresa, v.ubicacion, v.tipo]
      .join(" ")
      .toLowerCase()
      .includes(q)
  );
